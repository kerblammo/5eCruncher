...
<div class="entity">
  ...
  table.children[table.children.length - 1].insertAdjacentElement(
    "beforebegin",
    row
  );
  row
    .querySelector(".remove-row")
    .addEventListener("click", removeRowClickHandler);

  row
    .querySelector(".decrease-actors")
    .addEventListener("click", decreaseActorClickHandler);
  row
    .querySelector(".increase-actors")
    .addEventListener("click", increaseActorClickHandler);
  row
    .querySelector(".decrease-level")
    .addEventListener("click", decreaseLevelClickHandler);
  row
    .querySelector(".increase-level")
    .addEventListener("click", increaseLevelClickHandler);
}
...
function updateMonsterDifficulty(xp, thresholds) {
  ...
  let multiplier = MONSTERS.getDifficultyMultiplier(
    partyMemberCount,
    monsterCount
  );
